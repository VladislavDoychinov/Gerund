...
export default function SideBar({ position }: { position: LatLngExpression | null }) {
  ...
  const favouritePins = allPins.filter((p) => p.favouritedBy?.includes(currentUser));
  ...
  const PinCard = ({ pin }: { pin: Pin }) => {
    ...
    return (
      <li className={`mp-pin-card ${isFavourited ? "mp-pin-card--fav" : ""}`} style={{ borderLeftColor: pin.color || "#3B82F6" }}>
        <div className="mp-pin-card-header">
          ...
          <button
            ...
            title={isFavourited ? "Remove from favourites" : "Save as favourite"}
          >
            {isFavourited ? "★" : "☆"}
          </button>
        </div>
      ...
      <ul className="mp-sidebar-list">
        {SIDE_ITEMS.map((item) => {
          const badge =
            ...
            item === "Saved Places" ? favouritePins.length :
            null;
          return (
            <li key={item}>
              <button className={`mp-side-btn ${active === item ? "mp-side-btn--active" : ""}`} onClick={() => setActive(item)}>
