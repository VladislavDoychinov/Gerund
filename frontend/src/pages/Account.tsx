...
const DEMO_ACCOUNT_KEY = "example";

const DEMO_USER: UserRecord = {
  id: -1,
  username: "Example Vendor",
  name: "Example Vendor",
  email: "example@pulsepoint.demo",
};
...
const DEMO_PRODUCTS: ProductRecord[] = [
  {
    ...
    description: "Fresh farm tomatoes harvested this morning.",
    quantity: 20,
    category: "vegetables",
  ...
  },
  {
    ...
    description: "Juicy and sweet, sold in small baskets.",
    quantity: 12,
    category: "fruits",
    createdByEmail: DEMO_USER.email,
  },
...
export default function AccountPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (accountKey === DEMO_ACCOUNT_KEY) {
        const descriptionStorageKey = `account-description:${DEMO_USER.email.toLowerCase()}`;
        const savedDescription =
          localStorage.getItem(descriptionStorageKey) ||
          "This is a sample account used to preview the account page layout.";
        setAccountUser(DEMO_USER);
        setProducts(DEMO_PRODUCTS);
        setDescription(savedDescription);
        setDescriptionDraft(savedDescription);

        try {
          const meResponse = await api.get("/api/auth/me", { withCredentials: true });
        ...
        {!loading && !accountUser && (
          <section className="account-card">
            ...
            <p className="account-subtitle">
              Try the demo page: <Link to="/account/example">/account/example</Link>
            </p>
                    ...
                    <article key={product.id} className="account-product-card">
                      ...
                      <div className="account-product-body">
                        ...
                        {product.id > 0 ? (
                          ...
                          <span className="account-product-link">Preview product</span>
                        )}
                      </div>
                    </article>
                  ))}
